//Macro_Fiji_Brightfield_Segmentation_Images
macroName = "Macro_Fiji_Brightfield_Segmentation_Images";
macroShortDescription = "This macro calculates from brightfield / lipid droplet images new image for segmentation and saved them into a new folder.";
macroDescription = "This macro opens the images as an image sequence." +
	...
	"<br>- Gaussian blur (before and after variance filter), image multiplication factor and variance filter can be set.";
macroRelease = "first release 23-04-2019 by Martin Stöter (stoeter(at)mpi-cbg.de)";
...
firstGaussianRadius = 5;
secondGaussianRadius = 5;
multiplicationFactor = 10;
varianceFilterRadius = 30;
fileTag = "_IJ-BFseg";
renameFiles = true;

Dialog.create("Settings for movie generation");
Dialog.addString("Regular expresion", regexString);
Dialog.addNumber("Size of first Gaussian filter", firstGaussianRadius);
Dialog.addNumber("Size of second Gaussian filter", secondGaussianRadius);
Dialog.addNumber("Image multiplication factor", multiplicationFactor);
Dialog.addNumber("Size of variance filter", varianceFilterRadius);
...
firstGaussianRadius = Dialog.getNumber();
secondGaussianRadius = Dialog.getNumber();
multiplicationFactor = Dialog.getNumber();
varianceFilterRadius = Dialog.getNumber();
fileTag = Dialog.getString();
renameFiles = Dialog.getCheckbox();
hideImages = Dialog.getCheckbox();
print("Regular expresion", regexString, "; first Gaussian filter", firstGaussianRadius, "; second Gaussian filter", secondGaussianRadius, "image multiplication factor", multiplicationFactor, "variance filter", varianceFilterRadius, "; file tag for result images", fileTag, "; rename files?", renameFiles);
...
print("calculating new image from ", nSlices, "images...");
run("Multiply...", "value=" + multiplicationFactor + " stack");
run("Gaussian Blur...", "sigma=" + firstGaussianRadius + " stack");
run("Variance...", "radius=" + varianceFilterRadius + " stack");
run("Gaussian Blur...", "sigma=" + secondGaussianRadius + " stack");
print("saving images...");
